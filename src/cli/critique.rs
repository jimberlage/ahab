...
pub async fn critique(session_id: String) -> Result<()> {
    let home_dir = dirs::home_dir()
        .ok_or_else(|| AhabError::Config("Could not find home directory".to_string()))?;
    ...
    let status = Command::new("opencode")
        ...
        .status()
        .map_err(|e| {
            AhabError::Config(format!(
                "Failed to launch opencode: {}. Make sure opencode is installed and in your PATH.",
                e
            ))
        })?;

    if !status.success() {
        return Err(AhabError::Config(format!(
